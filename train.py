...
def train(dataset_dir):
    ...
    def collate_fn(batch):
        ...
        data = [np.pad(trj, ((0, max_len - len(trj)), (0, 0)), mode='edge') for trj in data]
        data = torch.tensor(np.stack(data), dtype=torch.float32).to(device)
        labels = torch.tensor(labels, dtype=torch.long)
        return data, labels

    dataset = DataLoader(LabelFreeDataset(dataset_dir), batch_size=64, shuffle=True, collate_fn=collate_fn)
