...
def make_env(env_name, *args, **kwargs):
    def thunck():
        if env_name == 'ant-dir':
            ...
            env = gym.make('ant-dir', task=0*np.pi, include_cfrc_ext_in_observation=False, terminate_when_unhealthy=False, *args, **kwargs)
        elif env_name == 'point-robot':
            env = gym.make('point-robot', task=np.pi/2, *args, **kwargs)
        else:
